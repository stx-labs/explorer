...
- [x] Evaluation harness (`scripts/tx-diagnosis/`, `pnpm diagnosis:eval` / `diagnosis:promote`):
      live sample of recent failures stratified per contract/function/result, deterministic rubric
      shared with the golden test (`eval/rubric.ts`), optional capped LLM judge, `--cases` +
      `--baseline` diff, promote-to-golden with drafted labels. Decided 2026-09-03: scripts only (no
      scheduled job), live samples rather than committed sample fixtures, judge opt-in
